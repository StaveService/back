...
RSpec.describe "/composer_musics", type: :request do
  ...
  context "POST /composer_musics" do
    context "with Authorization header" do
      ...
      it do 
        post user_music_composer_musics_path(user_id: user.id, music_id: music.id), params: { composer_music: artist.attributes }, headers: @headers
        expect(response).to have_http_status :created
      end
    end
    it "without Authorization header" do
      post user_music_composer_musics_path(user_id: user.id, music_id: music.id), params: { composer_music: artist.attributes }
